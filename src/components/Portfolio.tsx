...
export function Portfolio() {
    ...
    return (
      <section id="portfolio" className="section">
        <div ref={stackRef} className="shell">
          <div data-reveal><Eyebrow>{portfolio.kicker}</Eyebrow></div>
          <h2 data-reveal className="t-h2 mt-4">{portfolio.h2}</h2>
          <p data-reveal className="t-lead mt-5 max-w-[40rem]">{portfolio.sub}</p>
          <div className="mt-12 grid gap-4 sm:grid-cols-2">
            {portfolio.ventures.map((v) => (
              <div data-reveal key={v.name}><VentureCard v={v} /></div>
            ))}
          </div>
        </div>
      </section>
  ...
  return (
    <section id="portfolio" ref={section} className="relative bg-surface/40">
      <div ref={pinned} className="flex h-screen flex-col justify-center overflow-hidden">
        <div className="shell flex items-end justify-between gap-6 pb-8">
          <div>
            <Eyebrow>{portfolio.kicker}</Eyebrow>
            <h2 className="t-h2 mt-4">{portfolio.h2}</h2>
            <p className="t-lead mt-4 max-w-[34rem]">{portfolio.sub}</p>
          </div>
        ...
        <div ref={track} className="flex gap-5 pl-[max(4vw,calc((100vw-var(--shell))/2))] pr-[6vw]" style={{ willChange: 'transform' }}>
          ...
          <div className="flex w-[min(60vw,300px)] shrink-0 items-center">
            <p className="t-h3 max-w-[14ch] text-ink-faint">Six companies and counting.</p>
          </div>
        </div>
      </div>
    </section>
